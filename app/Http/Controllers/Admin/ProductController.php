...
//  dd($request->all());
        ...
        // Set productID for validation (null for create, actual ID for update)
        $productId = $action == "create" ? null : $request->productID;
    ...
    // Print barcode for single product
    public function printBarcode($id)
    {
        $product = Product::with('category')->findOrFail($id);
        return view('admin.product.print-barcode', compact('product'));
    }
    ...
    public function printBulkBarcodes(Request $request)
    {
        ...
        if (empty($productIds)) {
            return redirect()->back()->with('error', 'Please select products to print barcodes.');
        }
        ...
        return view('admin.product.print-bulk-barcodes', compact('products'));
    }
