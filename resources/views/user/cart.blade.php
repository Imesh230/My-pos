                    ...
                    <div class="bg-light rounded">
                        <div class="p-4">
                            ...
                            <div class="d-flex justify-content-between mb-4">
                                ...
                                <p class="mb-0" id="subTotal">{{ $totalPrice }} Rs</p>
                            </div>
                            <div class="d-flex justify-content-between">
                                <h5 class="mb-0 me-4">Deli fees:</h5>
                                <div class="">
                                    <p class="mb-0">500 Rs</p>
                                </div>
                            </div>
                        ...
                        <div class="py-4 mb-4 border-top border-bottom d-flex justify-content-between">
                            ...
                            <p class="mb-0 pe-4" id="finalFee">{{ $totalPrice + 500 }} Rs</p>
                        </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('.btn-plus').click(function(){
                ...
                $price = $parentNode.find("#price").text().replace("Rs","");
                $qty = $parentNode.find('#qty').val();
                // console.log($price, $qty);
                $totalPrice = $qty * $price;
                $parentNode.find('#eachTotal').html( $totalPrice + 'Rs' );
                finalCalculation();
        ...
        $('#proceedCheckOut').click(function(){
            ...
            $userId = $(".userId").val() * 1;
            // $totalPrice = $('#finalFee').text().replace("Rs","") * 1;


            $(" #dataTable tbody tr ").each(function( item, row){
                    $productId = $(row).find('.productId').val();
                    $qty = $(row).find('#qty').val() * 1;
                    $totalPrice = $(row).find('#eachTotal').text().replace("Rs", "") * 1;
        ...
        function finalCalculation(){
                $totalPrice = 0;
                $(" #dataTable tbody tr ").each(function( item, row){
                    $totalPrice += Number( $(row).find("#eachTotal").text().replace("Rs",""));
                })

            // console.log($totalPrice);
            $("#subTotal").html( `${$totalPrice} Rs `)
            $("#finalFee").html( `${$totalPrice+500} Rs`)
